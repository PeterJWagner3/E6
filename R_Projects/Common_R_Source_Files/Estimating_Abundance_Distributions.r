...
bH <- c(w$par,hS,w$value);
names(bH) <- c("Zipf_log-log_decay","Zipf_S","Zipf_lnL");
return(bH)
}
...
while (incr>0)	{
	...
	if (is.null(all_results))	{
		...
		results <- sapply(hS,optimize_zipf_abundance_given_hS,counts=counts[counts>0],observed=observed,max_zipf=max_zipf,zg=all_results$`Zipf_log-log_decay`[match(max(all_results$Zipf_lnL),all_results$Zipf_lnL)]);
...
bH <- as.vector(all_results[mlr,]);
bH[1] <- round(as.numeric(bH[1]),6);
bH <- c(bH,modified_AIC(max(all_results$Zipf_lnL),k=2,n=nspec));
...
names(bH) <- c("Zipf_Mandel_log-log_decay","Zipf_Mandel_Beta","Zipf_Mandel_S","Zipf_Mandel_lnL");
#print(bH)
return(bH)
}
...
# asm <- asm1[asm1 %in% asm2][1];
test_abundance_distributions <- function(asm,abundance_data,tests,assemblages,assemblage_names,temporary_output=TRUE,study_name="",byte_trail=FALSE,span=4,max_J=MAX_J,apprise=F,output_append=T,provide_saturated=TRUE,analysis_name="")	{
...
if (!is.na(match("Lognormal",tests)))	{
	...
	print(status)
	bst_lgnr <- optimize_lognormal_abundance(counts,span=span);
...
if (!is.na(match("Zipf",tests)))	{
	...
	print(status)
	bst_zipf <- optimize_zipf_abundance(counts,span=span);
	summary_stats <- c(summary_stats,bst_zipf)
	ct <- ct+1
	if (ct==1)	{
		AICcs <- bst_zipf[4]
		}	else {
		AICcs <- c(AICcs,as.numeric(bst_zipf[4]));
		names(AICcs[length(AICcs)]) <- names(bst_zipf)[4];
		}
...
#		print(names(summary_stats));
		ss <- data.frame(array(summary_stats,dim=c(1,length(summary_stats))),stringsAsFactors = hell_no);
#		ss <- rbind(ss,ss);
#		print(names(summary_stats));
		colnames(ss) <- names(summary_stats);
		for (i in 1:ncol(ss))	ss[,i] <- unlist(ss[,i]);
#		ss <- array(summary_stats,dim=c(1,length(summary_stats)));
		if (file.exists(output_file))	{
			sss <- read.csv(output_file,header=T);
			colnames(sss) <- colnames(ss);
#			if sum(colnames(ss) %in% colnames(sss))
			sss <- as.data.frame(rbind(sss,ss));
			write.csv(sss,output_file,row.names = F);
			} else	{
			write.csv(ss,output_file,row.names = F);
...
if (gsub(".csv","",assemblage_names_file)!=assemblage_names_file)	{
	assemblage_names <- read.csv(file=assemblage_names_file, header=T, stringsAsFactors=T);
	} else	{
...
output_summary <- pbapply::pbsapply(asm,test_abundance_distributions,abundance_data=abundance_data,tests=tests,assemblages=assemblages,assemblage_names=assemblage_names,temporary_output=temporary_output,byte_trail=byte_trail,span=span,max_J=MAX_J,apprise=apprise,provide_saturated=provide_saturated);
...
find_best_relative_abundance_models_w_PBDB_finds <- function(pbdb_finds_w_abundance,tests,min_counts,temporary_output=FALSE,study_name="",byte_trail=F,span=4,max_J=MAX_J,apprise=F,provide_saturated=F,analysis_name)	{
...
assemblage_names <- unique(abundance_data$collection_no);
