...
if (inc_children)	{
	httpT <- paste("https://paleobiodb.org/data1.2/taxa/list.csv?id=var:",taxon_id,"&rel=all_children&show=attr,common,app,parent,immparent,size,class,classext,subcounts,ecospace,taphonomy,pres,ref,refattr",sep="");
	} else if (inc_parents)	{
	httpT <- paste("https://paleobiodb.org/data1.2/taxa/list.csv?id=var:",taxon_id,"&rel=all_parents&show=attr,common,app,parent,immparent,size,class,classext,subcounts,ecospace,taphonomy,pres,ref,refattr",sep="");
	} else	{
	httpT <- paste("https://paleobiodb.org/data1.2/taxa/single.csv?id=var:",taxon_id,"&show=attr,common,app,parent,immparent,size,class,classext,subcounts,ecospace,taphonomy,pres,ref,refattr",sep="");
	}
taxon_information <- read.csv(httpT,header=T,stringsAsFactors=hell_no,encoding="UTF-8");
taxon_information <- expello_na_from_matrix(data=taxon_information);
return(taxon_information);
