...
Phanerozoic_Timescale_Plot_Flexible <- function(onset=-541,end=0,time_scale_to_plot,mxy,mny,use_strat_labels=TRUE,strat_names,strat_colors,plot_title="",ordinate="",abscissa="Ma",yearbreaks,xsize=6, ysize=4.285714285,hues=TRUE,colored="base",alt_back=FALSE,alt_back_hue="gray90",strat_label_size=1,xlab_off=2,ylab_off=2,font="")	{
# UPDATED 2016-12-31
...
if (font!="")	{
	title(xlab=abscissa, line=xlab_off, cex.lab=1.0,family=font);
	title(ylab=ordinate, line=ylab_off, cex.lab=1.0,family=font);
	}
...
for (i in 1:draws)	{
	...
	if (i < draws)	{
		...
		if (font=="")	{
			...
			axis(1,at=tcs,tcl=pts[i],labels=abs(tcs),lwd=0.0,lwd.ticks=4/3,family=font);
			}
		}
	}
...
specify_basic_plot <- function(mxx, mnx, mxy, mny, main="",subtitle="",abcissa="", ordinate="", xsize=3, ysize=3, cexaxis=1, cexlab=1, cexmain=1, cexsub=1,font="")	{
par(pin=c(xsize,ysize));
if (font=="")	{
	...
	plot(NA,type='n',axes=FALSE,main=main,sub=subtitle,xlab=abcissa,ylab=ordinate,xlim=c(mnx,mxx),ylim=c(mny,mxy),cex.axis=cexaxis,cex.lab=cexlab,cex.main=cexmain,cex.sub=cexsub,family=font);
	}
}
...
specified_axis <- function(axe,max_val,min_val,maj_break,med_break,min_break,linewd=4/3,font_size=1,orient=1,print_label=TRUE,font="")	{
#axis(axe,at=seq(min_val,max_val,by=max_val-min_val),tcl=-0.0,labels = FALSE,lwd=linewd,las=orient)
...
if (font=="")	{
	...
	axis(axe,at=ticks,tcl=-0.30,labels=print_label,lwd=0.0,lwd.ticks=linewd,las=orient,cex.axis=font_size,family=font);
	}
if (med_break!=0)	{
...
specified_axis_w_labels <- function(axe,max_val,min_val,maj_break,med_break,min_break,axis_labels,linewd=4/3,label_pos="tick",font_size=1,orient=1,print_label=T,font="")	{
#axis(axe,at=seq(min_val,max_val,by=max_val-min_val),tcl=-0.0,labels = FALSE,lwd=linewd,las=orient)
if ((min_val/maj_break)-floor(min_val/maj_break)<(10^-10))	{
	# this is a kluge necessitated by tiny rounding errors....
	mnv1 <- min_val;
...
specified_right_y_axis <- function(mxy1,mny1,max_val,min_val,maj_break,med_break,min_break,linewd=4/3,orient=2,print_label=TRUE,font="")	{
...
if (print_label)	{
	if (font=="")	{
		...
		axis(side=4,at=ticks,tcl=-0.30,labels=seq(mnv1,max_val,by=maj_break),lwd=0.0,lwd.ticks=linewd,las=orient,family=font);
		}
	} else	{
	if (font=="")	{
		axis(side=4,at=ticks,tcl=-0.30,labels=print_label,lwd=0.0,lwd.ticks=linewd,las=orient);
		} else	{
		axis(side=4,at=ticks,tcl=-0.30,labels=print_label,lwd=0.0,lwd.ticks=linewd,las=orient,family=font);
		}
	}
...
draw_calibrated_phylogeny_flex <- function(vtree,finds,durations,apos,orientation="vertical",oldest=NA,youngest=NA,taxon_labels=NA,otu_cols,lazarus_col="gray50",branching_col="black",plot_stratigraphy="ranges",new_plot=F,xsize=4,ysize=6,font="")	{
# draws phylogeny onto an already configured plot
notu <- match(-1,vtree)-1;
durations <- -abs(durations);
finds <- -abs(finds);
...
sampled_ancestors <- accersi_poss_ancestors_for_nodes(vector_tree=vector_tree,FA=durations[,1],apos=apomorphies);
...
draw_calibrated_phylogeny_new <- function(vector_tree,finds,durations,phylo_axis,apomorphies,orientation="vertical",taxon_labels="",otu_cols,lazarus_col="gray50",plot_stratigraphy=F,branching_col="black",lineage_lwd=4,lazarus_lwd=3,branching_lwd=2,lazarus_lty=1,taxon_label_cex=0.75,new_plot=F,mxz=0,mnz=0,height=4,width=6,max_age=NULL)	{
# modified 2022-09-24
# working as of 2019-07-10
# draws phylogeny onto an already configured plot or makes a new one
...
if (sum(durations$onset)>0)	finds <- -abs(finds);
...
get_phylo_axis_from_stratified_matrix_tree <- function(matrix_tree,sampled_ancestors,root_low=T)	{
# sampled_ancestors: vector of 0 & 1, with 1 = sampled ancestor
...
if (root_low)	{
	max_tax <- (1:notu)[match(max(otu_order),otu_order)];
	end_node <- which(matrix_tree==max_tax,arr.ind=T)[,1];
	if (end_node < (nNodes/2))	{
		otu_order <- notu-otu_order
		}
	} else	{
